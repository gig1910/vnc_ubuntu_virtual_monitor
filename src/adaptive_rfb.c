...
find_state_for_screen(rfbScreenInfoPtr screen)
{
    ...
    for (
        AdaptiveRfbState *state = adaptive_states;
        state;
    ...
    int encoding)
{
    ...
    AdaptiveRfbState *state =
        find_state_for_screen(cl ? cl->screen : NULL);
    ...
    if (newly_enabled) {
        fprintf(
            stderr,
            "[ADAPT] client advertised RFB JPEG encoding 21; "
            "adaptive JPEG enabled\n"
        );
    }
...
region_area(sraRegionPtr region)
{
    ...
    sraRectangleIterator *iterator =
        sraRgnGetIterator(region);
    sraRect rect;

    while (
        iterator &&
        sraRgnIteratorNext(iterator, &rect)
    ) {
        if (
            rect.x2 > rect.x1 &&
            rect.y2 > rect.y1
...
get_jpeg_client_ref(AdaptiveRfbState *state)
{
    ...
    rfbClientIteratorPtr iterator =
        rfbGetClientIterator(state->screen);
    rfbClientPtr cl;

    while (
        iterator &&
        (cl = rfbClientIteratorNext(iterator)) != NULL
    ) {
        if (cl == target) {
            rfbIncrClientRef(cl);
            found = cl;
    ...
    int y2)
{
    ...
    /* New lossy areas should first reappear across the whole image. */
    state->repair_pass = 0;
    state->repair_scan = 0;
}

static int
repair_tile_matches_pass(
    int tx,
    int ty,
    int pass)
{
    ...
    int *pass_out)
{
    ...
    for (int pass_attempt = 0; pass_attempt < 4; pass_attempt++) {
        ...
        for (
            size_t index = state->repair_scan;
            index < state->tile_count;
            index++
        ) {
            if (!state->repair_bits[index])
            ...
            int tx =
                (int)(index % (size_t)state->tiles_x);
            int ty =
                (int)(index / (size_t)state->tiles_x);
...
{
    AdaptiveJpegError *error =
        (AdaptiveJpegError *)cinfo->err;
    ...
    *jpeg_size = 0;
    ...
    while (cinfo.next_scanline < cinfo.image_height) {
        ...
        const uint8_t *src =
            ...
            (size_t)source_y *
                (size_t)screen->paddedWidthInBytes +
            (size_t)x * 4;

        for (int px = 0; px < width; px++) {
            /* Current server framebuffer is little-endian BGRx. */
    ...
    unsigned long *encoded_size)
{
    ...
    if (
        ...
        fprintf(stderr, "[ADAPT] JPEG encoder failed\n");
        return -1;
    }

    if (jpeg_size > (unsigned long)INT_MAX) {
        fprintf(stderr, "[ADAPT] JPEG rectangle is too large\n");
        free(jpeg_data);
        return -1;
    }
    ...
    rect.encoding =
        Swap32IfLE(ADAPTIVE_RFB_JPEG_ENCODING);
    ...
    int result)
{
    ...
    if (state) {
        ...
        previous =
            state->previous_display_finished_hook;
    ...
    if (log_repair) {
        fprintf(
            stderr,
            "[REPAIR] exact pass=%d tile=%d,%d remaining=%zu\n",
            pass,
            tx,
            ty,
            remaining
    ...
    int height)
{
    ...
    state->tile_count =
        (size_t)state->tiles_x *
        (size_t)state->tiles_y;

    state->repair_bits =
        calloc(state->tile_count, 1);
    ...
    pthread_once(
        &adaptive_extension_once,
        register_adaptive_extension
    );

    state->previous_display_finished_hook =
        screen->displayFinishedHook;
    screen->displayFinishedHook =
        adaptive_display_finished;
    ...
    fprintf(
        stderr,
        "[ADAPT] JPEG21 threshold=%.1f%% quality=%d; "
        "repair=%dpx progressive budget=%dkbit/s idle=%dms\n",
...
adaptive_rfb_destroy(AdaptiveRfbState *state)
{
    ...
    if (
        state->screen &&
        state->screen->displayFinishedHook ==
            adaptive_display_finished
    ) {
        state->screen->displayFinishedHook =
            state->previous_display_finished_hook;
    ...
    double now)
{
    ...
    if (
        ...
        /*
         * Consume exactly the readiness represented by requestedRegion.
         * The next server update again requires a fresh client request,
         * exactly like LibVNCServer's normal framebuffer-update path.
         */
    ...
    if (send_rc < 0) {
        fprintf(
            stderr,
            "[ADAPT] JPEG21 send failed; closing backend client "
    ...
    /*
     * Queueing and sending are driven by the backend publisher thread, so the
     * pending generation cannot normally change here. Keep the guard anyway
     * so a disconnect/reset never clears a newer generation accidentally.
     */
    if (
        state->pending_jpeg &&
        state->pending_sequence == sequence
    ...
    fprintf(
        ...
        "bytes=%lu raw=%llu ratio=%.1f%% repair=%zu\n",
    ...
    int y2)
{
    ...
    for (int ty = ty1; ty <= ty2; ty++) {
        int tile_y1 =
            ty * ADAPTIVE_RFB_REPAIR_TILE_SIZE;
        int tile_y2 =
            tile_y1 + ADAPTIVE_RFB_REPAIR_TILE_SIZE;
        ...
        for (int tx = tx1; tx <= tx2; tx++) {
            int tile_x1 =
                tx * ADAPTIVE_RFB_REPAIR_TILE_SIZE;
            int tile_x2 =
                tile_x1 + ADAPTIVE_RFB_REPAIR_TILE_SIZE;

            if (tile_x2 > state->width)
                tile_x2 = state->width;

            /*
             * A repair tile becomes exact only when this lossless rectangle
             * covers the complete tile. With the default 32px diff grid this
             * is exact, while partial future rectangles remain conservative.
             */
            if (
                x1 <= tile_x1 &&
                y1 <= tile_y1 &&
    ...
    double last_source_arrival)
{
    ...
    double quiet_anchor =
        last_source_arrival > state->last_lossy_at
            ? last_source_arrival
            : state->last_lossy_at;
    ...
    int eligible =
        ...
        quiet_anchor > 0.0 &&
        now - quiet_anchor >=
            ADAPTIVE_RFB_REPAIR_IDLE_SECONDS;
    ...
    fprintf(
        stderr,
        "[REPAIR] schedule pass=%d tile=%d,%d rect=%d,%d %dx%d "
        "remaining=%zu\n",
        pass,
        tx,
        ty,
...
adaptive_rfb_print_summary(AdaptiveRfbState *state)
{
    ...
    double saved =
        jpeg_raw_bytes > 0
            ? 100.0 -
                (double)jpeg_bytes * 100.0 /
                (double)jpeg_raw_bytes
            : 0.0;

    fprintf(
        stderr,
        "[ADAPT][SUMMARY] jpeg-updates=%llu jpeg-bytes=%llu "
        "raw-equivalent=%llu saved=%.1f%% repair-tiles=%llu "
        "repair-remaining=%zu jpeg-pending=%s\n",
